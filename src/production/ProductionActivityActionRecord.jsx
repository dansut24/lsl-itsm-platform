...
function ActionComposer({
  ...
  if (mode === 'attach') fields = <div className="activity-action-fields"><Field label="File" hint={file ? `${file.name} · ${formatBytes(file.size)}` : 'Maximum 5 MB'}><input type="file" onChange={(event) => setFile(event.target.files?.[0] || null)} /></Field></div>
