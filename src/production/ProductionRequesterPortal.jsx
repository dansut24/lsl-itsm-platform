...
export function ProductionRequesterPortal({ catalogue, tenant }) {
  ...
  return <div className="prp-shell"><header className="prp-header"><div className="prp-brand"><img src="/hi5central-logo.png" alt="" /><span><strong>{tenant.companyName}</strong><small>Help Centre</small></span></div><nav><button className={section === 'home' ? 'active' : ''} onClick={() => navigate('home')}><Home size={17} /> Home</button><button className={section === 'catalogue' ? 'active' : ''} onClick={() => navigate('catalogue')}><PackageOpen size={17} /> Services</button><button className={section === 'requests' ? 'active' : ''} onClick={() => navigate('requests')}><Inbox size={17} /> My Requests</button></nav><div className="prp-user"><span>{initials(user.name)}</span><div><strong>{user.name}</strong><small>{user.email}</small></div><button title="Sign out" onClick={signOut}><LogOut size={17} /></button></div><button className="prp-mobile-menu" onClick={() => setMobileNav(true)}><Menu size={21} /></button></header>{mobileNav ? <div className="prp-mobile-nav"><div><strong>{tenant.companyName}</strong><button onClick={() => setMobileNav(false)}><X size={20} /></button></div><button onClick={() => navigate('home')}><Home size={18} /> Home</button><button onClick={() => navigate('catalogue')}><PackageOpen size={18} /> Services</button><button onClick={() => navigate('requests')}><Inbox size={18} /> My Requests</button><button onClick={signOut}><LogOut size={18} /> Sign out</button></div> : null}<main className="prp-content">{section === 'catalogue' ? <Catalogue items={catalogue.items || []} onSubmitted={(request) => { setSelectedRequest(request.reference || request.id); setSection('requests'); setRefreshKey((value) => value + 1) }} /> : section === 'requests' ? <Requests selectedReference={selectedRequest} onSelect={setSelectedRequest} refreshKey={refreshKey} /> : <section className="prp-home"><div className="prp-hero"><span>Hi {user.name?.split(' ')[0] || 'there'}</span><h1>What can IT help you with?</h1><p>Report incidents, request services, follow progress and keep every customer-visible update in one place.</p><div><button className="prp-primary" onClick={() => navigate('catalogue')}><Plus size={17} /> Make a request</button><button className="prp-secondary" onClick={() => navigate('requests')}><Inbox size={17} /> View My Requests</button></div></div><div className="prp-home-grid"><button onClick={() => navigate('catalogue')}><span className="prp-icon"><PackageOpen size={20} /></span><strong>Service catalogue</strong><p>Report issues or browse the services your organisation has published.</p><ChevronRight size={17} /></button><button onClick={() => navigate('requests')}><span className="prp-icon"><Inbox size={20} /></span><strong>My Requests</strong><p>See incidents, service requests, fulfilment and customer updates.</p><ChevronRight size={17} /></button><div><span className="prp-icon"><ShieldCheck size={20} /></span><strong>Private by default</strong><p>You can only see requests associated with your authenticated account.</p></div><div><span className="prp-icon"><CircleDollarSign size={20} /></span><strong>Trusted pricing</strong><p>Catalogue prices are recalculated by Hi5Central when you submit.</p></div></div></section>}</main></div>
}