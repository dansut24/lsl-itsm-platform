...
import { CALENDAR_DEFAULT_TODAY, calendarEventTypes } from '../../data/calendarData.js'
...
function startOfWeek(value) {
  const date = typeof value === 'string' ? parseIsoDate(value) : new Date(value)
  if (!date) return CALENDAR_DEFAULT_TODAY
...
function parseSla(value) {
  ...
  const base = parseIsoDate(CALENDAR_DEFAULT_TODAY)
  const wanted = WEEKDAY_LOOKUP[weekday[1].slice(0, 3).toLowerCase()]
  let delta = wanted - base.getDay()
  if (delta < 0) delta += 7
  return { date: addDays(CALENDAR_DEFAULT_TODAY, delta), time: `${pad(weekday[2])}:${weekday[3]}` }
}
...
function emptyDraft(date = CALENDAR_DEFAULT_TODAY) {
...
export function CalendarView({
  ...
  const [selectedDate, setSelectedDate] = useState(CALENDAR_DEFAULT_TODAY)
    ...
    <div className="calendar-view">
      ...
      <section className="calendar-toolbar">
        <div className="calendar-period-nav">
          ...
          <button className="calendar-today-button" onClick={() => setSelectedDate(CALENDAR_DEFAULT_TODAY)} type="button">Today</button>
        ...
        <section className="calendar-month-shell">
          ...
          <div className="calendar-month-grid">
            {period.days.map((date) => {
              ...
              return (
                <div className={`calendar-month-day ${inMonth ? '' : 'outside-month'} ${date === CALENDAR_DEFAULT_TODAY ? 'is-today' : ''} ${date === selectedDate ? 'is-selected' : ''}`} key={date}>
