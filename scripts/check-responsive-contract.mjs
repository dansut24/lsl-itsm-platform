...
expect(main.indexOf(correctionsImport) > main.indexOf(glassImport), 'Workspace Shell v3 corrections must remain the final workspace chrome authority')
...
expect(shell.includes('.production-workspace-tabs') && shell.includes('overflow-x: auto'), 'Workspace tabs must retain local horizontal scrolling')
...
expect(/\.production-workspace-tabs\s*\{[\s\S]*?width:\s*100% !important;[\s\S]*?overflow-x:\s*auto !important;/.test(corrections), 'Tab row must scroll inside the available viewport instead of passing under actions')
